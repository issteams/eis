...
class QueryNormalizer:
    ...
    def normalize(self, query: str) -> Query:
        ...
        terms = tuple(dict.fromkeys(t for t in re.findall(r"[\w-]+", normalized) if t not in _STOPWORDS))
...
class KeywordRetriever:
    ...
    def retrieve(self, query: Query, items: Iterable[ContextItem], *, limit: int = 20) -> list[ContextItem]:
        scored = [self._with_score(item, self.score(query, item)) for item in items]
        return sorted(scored, key=lambda item: (-item.score, -item.provenance.authority, str(item.id)))[:limit]
    ...
    def _with_score(item: ContextItem, relevance: float) -> ContextItem:
        ...
        return ContextItem(item.id, item.content, item.provenance, relevance, item.freshness, score, item.metadata)
...
class InMemoryRetriever:
    ...
    @staticmethod
    def _filter(items: Iterable[ContextItem], metadata: dict[str, object]) -> list[ContextItem]:
        return [
            item
            for item in items
            if all(item.metadata.get(key, item.provenance.metadata.get(key)) == value for key, value in metadata.items())
        ]
...
class ContextScorer:
    ...
    def score(self, item: ContextItem) -> float:
        ...
        return item.relevance * 0.55 + item.provenance.authority * 0.25 + item.freshness * 0.10 + priority * 0.10

    def rank(self, items: Iterable[ContextItem]) -> list[ContextItem]:
        ranked = [
            ContextItem(i.id, i.content, i.provenance, i.relevance, i.freshness, self.score(i), i.metadata)
            for i in items
        ]
        return sorted(ranked, key=lambda i: (-i.score, -i.provenance.authority, str(i.id)))
...
def deduplicate(items: Iterable[ContextItem]) -> list[ContextItem]:
    ...
    for item in items:
        ...
        if current is None or (item.provenance.authority, item.score) > (current.provenance.authority, current.score):
            best[key] = item
