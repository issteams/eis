...
from eis.models.errors import ModelConfigurationError, ModelRateLimitError, ModelUnavailableError, ModelValidationError
...
class OpenAICompatibleModel:
    ...
    def __init__(self, *, provider: str, base_url: str, api_key: str, model: str, timeout: float = 60.0) -> None:
    ...
    async def generate_structured(self, request: StructuredGenerationRequest) -> StructuredGenerationResponse:
        payload = self._chat_payload(request)
        payload["response_format"] = {"type": "json_schema", "json_schema": {"name": "eis_response", "schema": dict(request.schema)}}
        data = await self._post("/chat/completions", payload, request.timeout)
        content = data["choices"][0]["message"].get("content") or "{}"
        try:
            parsed = json.loads(content)
        except json.JSONDecodeError as exc:
            raise ModelValidationError("provider returned invalid structured JSON", provider=self.provider) from exc
        if not isinstance(parsed, dict):
            raise ModelValidationError("structured response must be a JSON object", provider=self.provider)
        return StructuredGenerationResponse(parsed, self._metadata, self._usage(data.get("usage")), data, request.trace_id)
    ...
    async def embed(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        return EmbeddingResponse(vectors, self._metadata, self._usage(data.get("usage")), request.trace_id)
    ...
    async def stream(self, request: GenerationRequest) -> AsyncIterator[str]:
        ...
        try:
            async with httpx.AsyncClient(timeout=request.timeout or self._timeout) as client:
    ...
    def _chat_payload(self, request: GenerationRequest) -> dict[str, Any]:
        ...
        payload: dict[str, Any] = {"model": request.model or self.metadata.model, "messages": messages}
        if request.temperature is not None:
            payload["temperature"] = request.temperature
        if request.max_tokens is not None:
            payload["max_tokens"] = request.max_tokens
        if request.tools:
            payload["tools"] = [{"type": "function", "function": {"name": t.name, "description": t.description, "parameters": dict(t.parameters)}} for t in request.tools]
        return payload
    ...
    async def _post(self, path: str, payload: dict[str, Any], timeout: float | None) -> dict[str, Any]:
        try:
            ...
            raise ModelUnavailableError("provider request timed out", provider=self.provider) from exc
        except httpx.HTTPError as exc:
            raise ModelUnavailableError("provider request failed", provider=self.provider) from exc
    ...
    async def _raise_for_status(self, response: httpx.Response) -> None:
        if response.status_code == 429:
            ...
            raise ModelRateLimitError("provider rate limit exceeded", retry_after=float(retry_after) if retry_after else None, provider=self.provider)
        if response.status_code >= 500:
            raise ModelUnavailableError(f"provider returned HTTP {response.status_code}", provider=self.provider)
        if response.status_code >= 400:
            raise ModelValidationError(f"provider returned HTTP {response.status_code}", provider=self.provider)
